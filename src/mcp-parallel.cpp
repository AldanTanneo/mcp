...
void print_arg() {
  ...
  outfile << "@@@ latex output  = " << (latex.length() > 0 ? latex : "no")
          << endl;
...
Formula learnHornLarge(ofstream &process_outfile, const Matrix &T,
  ...
  for (Row f : F) {
    ...
    if (eliminated == false)
      process_outfile << "+++ WARNING: vector " << f << " not elminated"
                      << endl;
  }
...
Formula learnBijunctive(ofstream &process_outfile, const Matrix &T,
  ...
  if (strategy == sEXACT) {
    ...
    for (Row f : F)
      if (sat_formula(f, B))
        process_outfile << "WARNING: vector " << f << " not elminated from F"
                        << endl;
    ...
    for (int j = 0; j < lngt; ++j) {
      ...
      clause[j] = lpos;
      if (!satisfied_by(clause, F) && satisfied_by(clause, T))
        B.push_back(clause);
...
Formula post_prod(ofstream &process_outfile, ofstream &latex_outfile,
  ...
  if (closure == clDHORN) {
    ...
    process_outfile << "+++ final dual Horn formula [" << schf.size()
                    << "] =" << endl;
    process_outfile << formula2string(A, schf) << endl;
  }
...
void OneToOne(ofstream &process_outfile, ofstream &latex_outfile,
              const int &i) {
  ...
  for (int j = 0; j < grps.size(); ++j) {
    ...
    if (closure == clDHORN) {
      process_outfile << "+++ swapping polarity of vectors and treating "
                         "swapped vectors as Horn"
                      << endl;
      T = polswap_matrix(T);
      F = polswap_matrix(F);
    }
    ...
    process_outfile << "+++ Section of groups T=" << grps[i]
                    << " and F=" << grps[j] << ":" << endl;
    // process_outfile << "    " << sect << endl;
    if (!disjoint) {
      process_outfile << "+++ Matrices <T> and F are not disjoint, therefore I "
                         "cannot infer a formula"
                      << endl
                      << endl;
    } else {
      ...
      // 	process_outfile << "+++ " << pcl_strg[closure] << "Closure(T|_A)
      // [" << HC.size() << "]"; 	if (display >= ySECTION) {
      // process_outfile << " = { " << endl; 	  process_outfile << HC;
      // process_outfile << "+++ }";
      // 	}
      // 	process_outfile << endl;
      // } else if (closure == clBIJUNCTIVE)
      // 	process_outfile << "+++ " << pcl_strg[closure] << "Closure not
      // computed" << endl;
      ...
      if (closure == clHORN || closure == clDHORN)
        ...
        if (formula.empty()) {
          process_outfile
              << "+++ bijunctive formula not possible for this configuration"
              << endl
              << endl;
          continue;
        }
...
void OneToAll(ofstream &process_outfile, ofstream &latex_outfile,
              const int &i) {
  // one group of positive exaples against all other groups together as negative
  // examples
  ...
  for (int j = 0; j < grps.size(); ++j) {
    if (j == i)
      continue;
    F.insert(F.end(), group_of_matrix[grps[j]].begin(),
             group_of_matrix[grps[j]].end());
    index.push_back(grps[j]);
  }
  sort(index.begin(), index.end());

  if (closure == clDHORN) {
    process_outfile << "+++ swapping polarity of vectors and treating swapped "
                       "vectors as Horn"
                    << endl;
    T = polswap_matrix(T);
    F = polswap_matrix(F);
...
void OneToAllNosection(ofstream &process_outfile, ofstream &latex_outfile,
                       const int &i) {
  // one group of positive exaples against all other groups together as negative
  ...
  // if (closure == clHORN || closure == clDHORN) {
  //   HC = HornClosure(T);
  //   process_outfile << "+++ " << pcl_strg[closure] << "Closure(T) [" <<
  //   HC.size() << "]"; if (display == ySHOW) {
  //     process_outfile << " = { " << endl;
  //     process_outfile << HC;
  //     process_outfile << "+++ }";
  //   }
  //   process_outfile << endl;
  // } else if (closure == clBIJUNCTIVE)
  //   process_outfile << "+++ " << pcl_strg[closure] << "Closure not computed"
  //   << endl;
  ...
  if (disjoint) {
    Formula formula;
    if (closure == clHORN || closure == clDHORN)
      formula = strategy == sEXACT ? learnHornExact(T)
