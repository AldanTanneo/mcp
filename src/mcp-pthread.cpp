...
void thread_split(ofstream thread_outfile[], ofstream thread_latexfile[],
  ...
  thread_outfile[rank] << endl
                       << "+++ Start output of thread " << rank << endl
                       << endl;
...
int main(int argc, char **argv) {
  ...
  for (int rank = 0; rank < grps.size(); ++rank)
    threads.push_back(
        thread(thread_split, thread_outfile, thread_latexfile, basename, rank));
